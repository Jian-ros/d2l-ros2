...
发布图片的格式，我们需要采用ros自带的传感器消息接口中的图片`sensor_msgs/msg/Image`数据类型，所以我们新的消息文件就是将两者合并起来即可，在`ROS2`中可以写做这样：
...
其实很简单，我们不直接使用std_msgs/String直接使用最下面一层的string。
...
小鱼是如何知道，std_msgs/String是由基础数据类型string组成的，其实可以下面的指令来查看
...
原来std_msgs的String就是一个叫data类型的string变量，这也是在4.2和4.3章节代码要用`.data`才能拿到真正的数据的原因：
...
小鱼这里选择第二种写法，让大家即了解ROS2的原始数据类型也了解ROS2自带的消息接口包。
