...
本章节中小鱼将带你一起，通过Python实现王二和李四之间的话题通信，完成李四发布话题（sexy_girl）,王二订阅话题（sexy_girl）。

大家都知道白嫖是不对的（看了小鱼视频一定要一键三连），所以李四还要订阅一个话题来收稿费（sexy_girl_money），王二要发布（sexy_girl_money）话题来支付稿费。

接下来和和小鱼一起动手学习编写Python的话题发布者和订阅者吧~
...
这是什么意思呢？如果没有学过面向对象的同学可以很懵逼，其实很简单，这种在自己名字里写一个Node的意思就是让Li4Node继承于Node，这样李四就能拥有Node所具备的属性和能力。

大家可以把Node理解成人类，Li4Node继承与Node,也就是说李四是一个人类,这样李四就拥有了人类所具备的手和脚、说话吃饭刷B站和关注小鱼公众号的能力了。
...
接下来我们就依次调用Li4Node所继承来的能力实现订阅发布功能。
...
小鱼这里使用create_publisher方法来创建的发布者，该方法一共有三个参数，第一个是方法类型，我们这里添了String，需要注意的是，这里的String并非Python自带的字符串类型，我们使用

```
from std_msgs.msg import String
```

从std_msgs.msg中导入了String类，那std_msgs是什么呢？

std_msgs是ros2自带的消息类型，其中规定了我们常用的大多数消息，可以使用下面的指令来查看std_msgs中所有的消息类型。
...
大家可以根据自己的传输需要选择自己的消息类型，比如我们接下来想让李四收钱，我们将消息类型设置为UInt32，无符号整型，毕竟收钱没有收成负的了。

> 还可以使用 `ros2 interface list`查看所有ros2自带的消息类型。
>
> 需要注意的是，ros2中自带的类型基本上能够满足我们日常做机器人时使用，但如果ros2中的消息类型不能满足我们的使用的时，也可以选择自己定义消息类型。
...
这个定时器的作用就是根据传入的`timer_period`时间周期，每`1/timer_period`秒，调用一次`self.timer_callback`函数。
...
这句话的意识是创建订阅者，订阅话题`sexy_girl_money`,话题类型为`UInt32`,每次收到钱就去调用`self.recv_money_callback`函数存起来。
...
至此，相信你已经掌握了，如何编写一个Python的节点并进行订阅和发布话题了。
